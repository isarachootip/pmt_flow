...
export default function GanttPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 space-y-4 p-6 overflow-y-auto">
      
      {/* Top Header Card */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-border-soft">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-indigo-500/15 text-indigo-600 flex items-center justify-center text-2xl font-bold border border-indigo-500/20 shadow-inner">
            <FolderKanban className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-black tracking-tight">แผนงาน Gantt & บันทึกงานช่างประจำวัน</h1>
            ...
            <p className="text-xs text-slate-600 mt-0.5 leading-relaxed">
              ติดตามไทม์ไลน์โครงการ 3 ระดับ (Project ➔ Area ➔ Task), ตรวจสอบความล่าช้า, และบันทึกรายงานช่าง 24 ชม. 5 รูป
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {/* Total Tasks */}
        <div className="p-3.5 rounded-xl bg-white border border-border-soft shadow-2xs flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center font-bold">
            <Layers className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[11px] text-slate-500 font-medium block">งานทั้งหมด</span>
            <span className="text-lg font-bold text-black font-mono">{stats.total}</span>
          </div>
        </div>
        ...
        <div 
          ...
          className={`p-3.5 rounded-xl bg-white border transition-all cursor-pointer flex items-center gap-3 ${
        ...
        >
          ...
          <div>
            <span className="text-[11px] text-blue-700 font-medium block">กำลังทำ (On Process)</span>
            <span className="text-lg font-bold text-blue-900 font-mono">{stats.onProcess}</span>
          </div>
        </div>
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-rose-100 text-rose-600 flex items-center justify-center font-bold animate-pulse">
            <AlertTriangle className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[11px] text-rose-700 font-bold block">ล่าช้า (Delay)</span>
            <span className="text-lg font-bold text-rose-900 font-mono">{stats.delayed}</span>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <span className="text-[11px] text-amber-800 font-medium block">รอตรวจ (Wait QC)</span>
            <span className="text-lg font-bold text-amber-950 font-mono">{stats.waitQc}</span>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <span className="text-[11px] text-emerald-700 font-medium block">เสร็จสิ้น (Passed)</span>
            <span className="text-lg font-bold text-emerald-950 font-mono">{stats.passed}</span>
          </div>
        </div>

        {/* Active Today */}
        <div className="p-3.5 rounded-xl bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 shadow-2xs flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-red-500 text-white flex items-center justify-center font-bold shadow-2xs">
            <Calendar className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[11px] text-red-800 font-bold block">📍 วันนี้ (Today)</span>
            <span className="text-lg font-bold text-red-900 font-mono">{stats.activeToday} งาน</span>
          </div>
        </div>
      </div>

      {/* Filter & Search Bar */}
      <div className="p-4 rounded-xl bg-white border border-border-soft shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3">
        {/* Search */}
        <div className="relative flex-1 w-full">
          ...
          <Input 
            ...
            className="pl-9 text-xs text-black font-medium"
          />
        </div>
      ...
      <div className="flex-1 min-h-[460px]">
        {isTasksLoading || isJobsLoading ? (
          <div className="p-8 bg-white rounded-2xl border border-border-soft space-y-4">
      ...
      <div className="bg-white border border-border-soft rounded-2xl shadow-sm p-5 space-y-4">
