...
// =============================================================================
async function dbLoadDailyWorkLogs() {
    if (!exports.isDatabaseConnected)
        return [];
    try {
        const res = await exports.pool.query('SELECT * FROM core_daily_work_logs ORDER BY log_date DESC, id DESC');
        return res.rows.map(row => ({
            ...row,
            photos: row.photos || []
        }));
...
// =============================================================================
async function dbLoadQCBookings() {
    if (!exports.isDatabaseConnected)
        return [];
    try {
        const res = await exports.pool.query('SELECT * FROM core_qc_bookings ORDER BY booking_date DESC, id DESC');
        return res.rows.map(row => ({
            ...row,
...
async function dbLoadMAContracts() {
    if (!exports.isDatabaseConnected)
        return [];
    try {
        const res = await exports.pool.query('SELECT * FROM ma_contracts ORDER BY contract_start_date DESC');
        return res.rows.map(row => ({
            ...row,
...
async function dbLoadMARounds() {
    if (!exports.isDatabaseConnected)
        return [];
    try {
        const res = await exports.pool.query('SELECT * FROM ma_rounds ORDER BY scheduled_date ASC');
        return res.rows;
    }
    catch (err) {
