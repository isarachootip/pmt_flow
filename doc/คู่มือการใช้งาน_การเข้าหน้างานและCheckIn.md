...
## 🔒 ข้อกำหนดมาตรฐานการตัดจบงาน "User ยืนยัน" (Completion & Early Finish Standard)

1. **การบันทึกสถานะจบงาน 100% (Completed / Early Finish):**
   - เมื่อช่างปฏิบัติงานเสร็จสมบูรณ์ทั้งสิ้น (ไม่ว่าจะเป็นวันสุดท้ายตามแผน หรือจบงานก่อนกำหนด Early Finish) และต้องการส่งต่องานให้ QC ตรวจสอบ
   - ระบบกำหนดให้ต้องผ่านการยืนยันว่า **"User ยืนยัน"** (ลูกค้ายืนยันรับมอบงาน หรือ User ผู้ใช้งานยืนยันความเรียบร้อย)
   - เมื่อกดปุ่ม **"🚀 ยืนยันสำเร็จ & ส่งตรวจ QC"** หรือติ๊กช่อง **"ช่างบันทึกสำเร็จ (User ยืนยัน)"**:
     - ระบบจะแสดงกล่องข้อความยืนยัน: *"ยืนยันว่า User ได้ตรวจสอบและยืนยันงานติดตั้งเสร็จสมบูรณ์ 100% แล้วใช่หรือไม่?"*
     - รายละเอียดงาน (Work Description) จะบันทึกคำว่า **`(User ยืนยัน)`** เสมอ เช่น *"งานติดตั้งเสร็จสมบูรณ์ 100% (User ยืนยัน) ตรวจสอบระบบเรียบร้อย พร้อมส่งมอบให้ทีม QC ตรวจรับรองคุณภาพ"*
     - การ์ดประวัติ (History Card) และแถบไทม์ไลน์รายวันจะติดป้ายสีเขียว: **`✓ ช่างบันทึกสำเร็จ (User ยืนยัน)`**
2. **ระบบคืนสถานะอัตโนมัติเมื่อลบบันทึก (Auto Rollback on Delete):**
   - หากผู้ใช้บันทึกงานผิดวันหรือเผลอกดจบงานก่อนเวลา แล้วกดไอคอน 🗑️ (ถังขยะ) เพื่อลบรายการนั้นออก
   - ระบบจะตรวจสอบบันทึกที่เหลืออยู่ และ **ถอยสถานะ Task และ Job กลับเป็น `IN_PROGRESS` อัตโนมัติ** พร้อมคำนวณเปอร์เซ็นต์ความคืบหน้าตามจำนวนวันที่เหลืออยู่จริง ไม่ค้างสถานะ 100% อีกต่อไป
