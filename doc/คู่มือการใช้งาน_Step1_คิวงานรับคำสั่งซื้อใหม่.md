...
| **ตัวกรองสถานะ** | Dropdown `📥 คิวงาน Step 1 (Order ใหม่ / Survey / Draft)` | กำหนดขอบเขตการแสดงผล ค่าเริ่มต้นคือแสดงเฉพาะคิว Step 1 (สถานะ Survey/Draft) หากเปลี่ยนเป็น `🗂️ ทั้งหมดทุกขั้นตอน` จะเห็นงานทั้งระบบ | ในการทำงานประจำวันให้เปิดไว้ที่ "คิวงาน Step 1" เสมอ |
| **ถอยสถานะเป็น Survey** | ปุ่มขอบส้ม `🔄 ถอยสถานะเป็น Survey` | รีเซ็ตสถานะงานทั้งหมดในระบบให้กลับมาเป็นสถานะเริ่มต้นสำรวจหน้างาน (Survey / 25%) พร้อมคงรูปถ่ายหน้างานไว้เพื่อให้ฝึกทำแบบและ BOQ | ใช้วนรอบการฝึกอบรม เพื่อให้ผู้เรียนเริ่มทำซ้ำได้ใหม่ |
...
| **จำลอง 10 งาน (INT)** | ปุ่มขอบม่วง `⚡ จำลอง 10 งาน (INT)` | จำลองการยิงข้อมูล Order ตัวอย่างจากระบบ INT เข้าสู่สถานะ Survey (25%) พร้อมภาพถ่ายสำรวจหน้างานจริง | ให้ผู้เรียนกดปุ่มนี้ 1 ครั้งเพื่อเริ่มแบบฝึกหัด |
| **รับ Order ใหม่ (INT)** | ปุ่มสีม่วงทึบ `➕ รับ Order ใหม่ (INT)` | เปิดหน้าต่าง One-Stop Order Studio แบบ Ultra-Wide เพื่อดูรูปสำรวจ, แนบแบบ และลง BOQ | ใช้บันทึกคำสั่งซื้อทั้งแบบ Manual และจากระบบภายนอก |
...
3. **Card 3 — รอรับเข้า PMT (`10 รายการ`):** งานที่ยังค้างอยู่ใน Step 1 และยังไม่ได้กดรับเข้าสู่ระบบ PMT **เป้าหมายประจำวันคือต้องเคลียร์ตัวเลขนี้ให้เหลือ `0`**
...
ตารางแสดงรายการงานใน Step 1 ประกอบด้วยคอลัมน์สำคัญ (จัดเรียง 4 คอลัมน์แรกตามมาตรฐานระบบ):

1. **เลขที่งาน (Job ID):** รหัสอ้างอิงของงาน เช่น `JOB26090900001`, `JOB26090900002` พร้อมป้ายสถานะ `NEW!`
2. **เลขที่อ้างอิง (Ref ID):** เลขที่เอกสารอ้างอิงภายนอกจากระบบ INT หรือระบบต้นทาง เช่น `EXT-9001`
3. **เลขที่ Booking:** รหัสการจองคิวงานจากระบบ Booking กลาง เช่น `BK-2026-001`
4. **กำหนดวันนัด:** กำหนดวันนัดหมายทำงาน แสดงผลตามมาตรฐานวันที่ **`DD/MM/YYYY`** (เช่น `18/09/2026`) พร้อมไอคอนปฏิทิน
5. **ลูกค้า & เบอร์ติดต่อ:** ชื่อ-นามสกุลลูกค้า และหมายเลขโทรศัพท์สำหรับติดต่อหน้างาน
6. **ประเภทบริการ:** Badge ระบุประเภท (`QUICK` สีเหลือง / `RENOVATE` สีฟ้า) พร้อมชื่อรายการสินค้า/บริการ
7. **แบบแปลน (Design):** Badge แสดงจำนวนแบบแปลนที่แนบไว้ เช่น `2 แบบ (CAD/PDF)` สีฟ้า หรือ `+ แนบแบบ` สีเทา **(คลิกเพื่อเปิด Studio แท็บที่ 2 ได้ทันที)**
8. **ถอดราคา (BOQ):** Badge แสดงยอดเงินรวมของ BOQ เช่น `฿145,000` สีเขียว หรือ `+ ลง BOQ` สีเทา **(คลิกเพื่อเปิด Studio แท็บที่ 3 ได้ทันที)**
9. **ช่าง / ผู้รับผิดชอบ:** ทีมช่างที่ถูกกำหนดไว้เบื้องต้น เช่น `Team A (สมศักดิ์)`, `Team B (ประเสริฐ)`
10. **ขั้นตอน & เวลา (STAGE):** แถบเปอร์เซ็นต์ `Step 1 (0%)`, สถานะ `รอรับเข้า PMT` และเวลานับถอยหลัง SLA
11. **การจัดการ (Smart Studio & Quick Actions):**
