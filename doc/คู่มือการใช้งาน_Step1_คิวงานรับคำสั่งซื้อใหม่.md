...
- **ข้อมูลที่ STAMP มาจาก INT (Strict Read-Only Guard):** การ์ดฝั่งซ้ายและขวามีป้ายกำกับ `STAMP ข้อมูลมาจาก INT` ฟิลด์สำคัญที่ถูกล็อกไม่ให้แก้ไขใน PMT เพื่อความถูกต้องของข้อมูลต้นทาง ได้แก่:
...
- **ประเภทบริการย่อย (Service / project_sub_type) & Manual Fallback:**
  - แสดงชื่อบริการเฉพาะทางจาก INT อัตโนมัติ พร้อมป้ายกำกับสีน้ำเงิน `STAMP จาก INT`
  - **ข้อยกเว้นกรอก Manual:** หากคำสั่งซื้อยังไม่ได้รับข้อมูลประเภทบริการย่อยจาก INT ระบบจะแสดงป้ายเตือนสีส้ม `กรอก Manual (ยังไม่ได้รับจาก INT)` และเปิดให้เจ้าหน้าที่พิมพ์ระบุรายละเอียดบริการ หรือเลือกจากรายการบริการที่กำหนด (Datalist Autocomplete) ได้ทันที เมื่อบันทึกจะซิงค์ลงฟิลด์ `project_sub_type` และ `service` ในฐานข้อมูล
...
> 2. **โครงการ Renovate (Full Studio):** แสดงผลแบบ Ultra-Wide เต็มรูปแบบครบทั้ง 3 มิติ (Intake & Survey Photos, Blueprints CAD, BOQ & Costing) พร้อมแถบนำทาง Sticky และเงื่อนไข Gating Rules สำหรับการเปิด Ticket และแปลงเข้าแผนงานต่อไป
