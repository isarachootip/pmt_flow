...
> [!IMPORTANT]
> **ขอบเขตและเงื่อนไขการเข้าสู่ Step 5 (Applicable Scope & Prerequisites):**  
> 1. **ต้องผ่าน Step 3 (BOQ) เรียบร้อยแล้วเท่านั้น:** โครงการที่จะส่งต่อหรือแปลงเข้าสู่ Step 5 ได้ ต้องมีรายการวัสดุ/ค่าแรงใน BOQ อย่างน้อย 1 รายการ หากยังไม่มีข้อมูล BOQ ปุ่มย้ายไป Step 5 ทั้งในหน้า One-Stop Studio และระบบคิวงานจะถูกล็อกสถานะ (`disabled = true`) ไม่สามารถคลิกข้ามขั้นตอนได้เด็ดขาด
> 2. **เฉพาะงานโครงการปรับปรุง (Renovate Projects):** หน้าจอ Step 5 นี้ถูกออกแบบมาสำหรับงานโครงการที่มีรายการ BOQ ที่ต้องแปลงเป็นแผนงานช่างใน Gantt Chart  
> 3. **งานบริการติดตั้งด่วน (Quick Services):** ระบบจะ **ข้าม Step 5 โดยอัตโนมัติ** ทันทีที่บันทึกชำระเงินและ Ticket ใน Step 4 เรียบร้อย โดยจะส่งตรงไปยัง **Step 6: ตรวจรับรองคุณภาพ (QC Online)** ทันที เพื่อความรวดเร็วในการส่งมอบงาน
...
1. **แกนเวลาและการระบุตำแหน่ง "วันนี้" (Realtime Time Axis & Today Indicator):**
   - แสดงคอลัมน์ปฏิทินรายวัน วันเริ่มต้นถึงสิ้นสุดโครงการ
   - มี **ป้ายกำกับ `[ วันนี้ DD ด.ด. ]`** บนส่วนหัวของคอลัมน์วันปัจจุบัน พร้อม **เส้นประสีแดงแนวตั้ง (Vertical Red Dashed Line)** และจุดมาร์กเกอร์สีแดงลากผ่านทุกแถวงาน เพื่อแยกอดีต ปัจจุบัน และอนาคตอย่างชัดเจน
   - มีปุ่มจำลองวัน (Simulation Toolbar): `14 ก.ย.`, `15 ก.ย.`, `16 ก.ย.`, `17 ก.ย.`, `วันจริง` เพื่อให้ผู้ตรวจสอบและเจ้าหน้าที่ทดสอบสถานะแบบ Real-time ได้
...
3. **ความหมายของสีแท่งงาน Real-time รายวัน (Daily Real-time Status Color Coding):**
   - 🟩 **สีเขียว (`✓ เสร็จสิ้น`):** วันที่ช่างหรือ QC เข้ามาอัปเดตงานและบันทึกผลเรียบร้อยแล้ว (จะขึ้นสีเขียวเฉพาะวันที่มีการอัปเดต)
   - 🟥 **สีแดง (`ยังไม่อัปเดต`):** วันทำงานตั้งแต่เริ่มจนถึง "วันนี้" ที่ยังไม่ได้ลงบันทึกงาน
   - 🟪 **สีม่วง (`X วัน` / `Xd`):** ช่วงเวลาในอนาคตที่ยังมาไม่ถึงตามแผนงาน (Unstarted Period)
