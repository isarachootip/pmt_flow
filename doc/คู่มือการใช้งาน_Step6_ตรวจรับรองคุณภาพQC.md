...
graph LR
    ...
    VP --> Review["เจ้าหน้าที่ QC ตรวจสอบรูปภาพ & ตอบแบบประเมิน 5 ข้อ"]
    Review -->|อนุมัติผ่าน 100%| CSAT["Step 6: CSAT Survey & ปิดงาน"]
```
...
- รูปภาพจะถูกจัดสรรเข้าสู่คำถามมาตรฐานของงานด่วนโดยอัตโนมัติ ให้ผู้ตรวจสามารถคลิกขยายดูรูปขนาดใหญ่ (Lightbox Preview) และตรวจสอบความเรียบร้อยได้ทันที

### 4. การประเมินและอนุมัติผ่านเกณฑ์
- ผู้ตรวจตอบคำถามมาตรฐาน 5 ข้อ Yes = 5 คะแนน และเลือกสถานะ `ผ่าน (PASS)` ในทุกข้อ
- บันทึกข้อเสนอแนะภาพรวม และกดปุ่ม **"✓ อนุมัติผ่านเกณฑ์ QC & ส่งข้อมูลไป STK"** เพื่อส่งผลตรวจ (เลขที่, คำถาม, คะแนน) ไปยังระบบภายนอก STK ทันทีผ่าน Outbound REST API
...
1. **การเลือกผลการประเมิน Yes / No (Yes = 5 คะแนน, No = 1 คะแนน):**
   ...
   - **ปุ่มลัด `[ ✓ เลือก Yes ทั้งหมด (ได้ 5 คะแนนทุกข้อ) ]`**: ช่วยให้เจ้าหน้าที่ QC ตอบรับรองทุกข้อได้ในคลิกเดียวเมื่อตรวจสอบแล้วผ่านเกณฑ์ทั้งหมด
2. **การคำนวณคะแนนเฉลี่ยรวม (Overall QC Score):**
   - คำนวณจากคะแนนรวมของข้อที่ตอบ / 5 ข้อ เช่น ตอบ Yes ทั้ง 5 ข้อ = 5.0 / 5.0 คะแนน
...
## 🔒 กฎเหล็กการส่งข้อมูลผลตรวจสู่ระบบ STK (Strict 100% Completion Gating Rule)

เพื่อรักษามาตรฐานคุณภาพงานและความสมบูรณ์ของข้อมูลก่อนส่งออกไปยังระบบภายนอก STK ระบบ PMT Flow จึงกำหนดเงื่อนไขการส่งข้อมูลอย่างเคร่งครัด:

> [!CAUTION]
> **เงื่อนไขบังคับในการส่งผลตรวจไป STK (Gating Rule):**  
> เจ้าหน้าที่ QC **ต้องทำการประเมินและตอบ Yes/No ให้ครบทั้ง 5 ข้อคำถาม (100% Completion) และต้องไม่มีข้อบกพร่อง (Defect)** ระบบจึงจะยอมให้กดปุ่มอนุมัติและยิง REST API ส่งข้อมูลผลตรวจไปยังระบบ STK ได้
...
2. **ปุ่มอนุมัติถูกล็อก (Disabled State):**
   - ตราบใดที่ยังตรวจไม่ครบ 5 ข้อ หรือมีข้อที่ตอบ No (ข้อบกพร่อง) ปุ่ม **"✓ อนุมัติผ่านเกณฑ์ QC & ส่งข้อมูลไป STK"** จะอยู่ในสถานะปิดการใช้งาน และแสดงข้อความเตือนให้แก้ไขงานหรือสั่ง Rework
3. **การปลดล็อกปุ่มอนุมัติ (Active State):**
   - เมื่อประเมินครบ 5 ข้อคำถาม และได้ผล Yes ทั้งหมด แถบสถานะจะเปลี่ยนเป็น `ประเมินครบ 5 ข้อ (ผ่านเกณฑ์ 100%)` และปุ่มอนุมัติจะเปลี่ยนเป็นสีเขียวสว่างสดใสพร้อมให้กดส่งข้อมูลไปยังระบบ STK ทันที
4. **โครงสร้างข้อมูล Outbound API ที่ส่งออกไปยังระบบ STK:**
   - **เลขที่ใบงาน (`job_no`):** เลขที่ใบงาน เช่น `JOB26091464044`
   - **รายการคำถาม (`questions`):** รายการคำถามมาตรฐานที่ประเมินทั้ง 5 ข้อ พร้อมหมวดหมู่ และหมายเหตุ
   - **คะแนนที่ได้ (`scores`):** คะแนนรายข้อ (Yes=5 คะแนน, No=1 คะแนน), ผลการตรวจ (PASS), คะแนนเฉลี่ยรวม (`qc_score`), คะแนนรวมที่ได้, และคะแนนเต็ม
5. **ผลลัพธ์หลังการอนุมัติ:**
