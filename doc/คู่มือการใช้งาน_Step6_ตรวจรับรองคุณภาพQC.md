...
หน้าจอ **ตรวจคุณภาพ (Step 5: QC - Quality Control Inspection)** คือ **"ประตูตรวจรับรองมาตรฐานวิศวกรรมและความปลอดภัย และบันทึกปิดงานส่งระบบ STK (Engineering Quality, Safety Gatekeeper & Official STK Job Closeout)"** ของระบบ PMT Flow มีหน้าที่รับช่วงต่องานเพื่อทำการตรวจสอบ ประเมินมาตรฐานแบบ **Zero-Defect Standard** บันทึกปิดงานอย่างเป็นทางการ และยิงข้อมูลผลประเมินเข้าสู่ระบบ STK ทันทีเพื่อจบขั้นตอนโครงการใน PMT Flow (โดยไม่มีการส่งต่อไปยัง CSAT ในระบบ เนื่องจาก STK รับช่วงดำเนินการต่อทั้งหมด)

ในเวอร์ชันปัจจุบัน ระบบได้ยกระดับความสามารถและแยกสายการทำงานออกเป็น **2 รูปแบบอย่างชัดเจน**:
1. **งานบริการด่วน (Quick Services):** ตรวจสอบผลงานผ่านรูปถ่ายหน้างาน (Photo Audit Online) เปรียบเทียบก่อน-หลัง พร้อมตอบแบบประเมิน 1 ข้อคำถามเดียวจบ และกดส่งผลตรวจไป STK เพื่อปิดงานทันที
2. **งานโครงการปรับปรุง (Renovate Projects):** มีความพิเศษและยืดหยุ่นสูง โดยฝ่าย QC จะดำเนินงานตามกระบวนการ:
   - **ตรวจสอบภาพถ่ายช่าง 5 รูป:** ตรวจสอบชุดภาพถ่าย 5 ใบ (Before, During 1-2, Test, After) ที่ช่างบันทึกไว้ใน Step 4 Daily Logs
   - **แยกตรวจและปิดงานตามงานหลัก (Per-Task QC Inspection & STK Submission):** หากโครงการมีหลายงานหลัก (เช่น งานทาสี, งานปูกระเบื้อง) จะแยกแท็บให้ QC ตรวจและปิดงานทีละงานหลักอย่างเป็นอิสระต่อกัน งานใดเสร็จก่อนส่งประเมิน STK ได้ทันที
   - **รวมงานย่อยเป็นหนึ่งในงานหลัก:** งานย่อย (Subtasks) และรายการค่าแรงจาก BOQ ถูกรวบรวมเข้ามาแสดงผลอยู่ภายใต้งานหลักนั้นๆ ไม่แยกตรวจเป็นเอกเทศ
   - **1 ข้อคำถามมาตรฐาน On-site + เพิ่มข้อคำถามได้เอง:** เริ่มต้นด้วยข้อคำถามมาตรฐาน On-site 1 ข้อ (*"ช่างปฏิบัติงานติดตั้งได้ตามแบบ BOQ และมาตรฐาน On-site ถูกต้องเรียบร้อย"*) พร้อมปุ่มให้ฝ่าย QC เพิ่มข้อคำถามประเมินได้เองตามความเหมาะสม
   - **ปิดงานราย Task ส่ง STK ทันที:** แต่ละงานหลักมีปุ่ม `✓ บันทึกปิดงาน [ชื่องานหลัก] & ส่งประเมิน STK` เมื่อกดจะยิงข้อมูลไปยัง STK ทันที (Ref: `STK-TASK-...`)
   - **จบโครงการ 100%:** เมื่องานหลักทั้งหมดส่ง STK ครบแล้ว โครงการจะปิดสมบูรณ์ 100% (`QC_PASSED` / `CLOSED`) โดยไม่มีการส่งต่อไปยัง CSAT ใน PMT Flow
...
3. **ใช้งานการประเมินและปิดงานรายงานหลัก (Per-Task QC & STK Submission):** ตรวจสอบรูปภาพช่าง 5 ใบ, ประเมินข้อคำถามมาตรฐาน 1 ข้อ + เพิ่มข้อคำถามได้เอง, รวมงานย่อยในงานหลัก, และปิดงานส่งข้อมูลไป STK แยกตาม Task
4. **ปฏิบัติตามเกณฑ์ Gating ส่งข้อมูลไป STK 100%:** เข้าใจเงื่อนไขความสมบูรณ์ในการตรวจรับรองผลงานก่อนยิง REST API ส่งข้อมูลผลตรวจไปยังระบบภายนอก STK
...
- **แถบค้นหาและตัวกรองข้อมูลหลายมิติ (QC Search & Multi-Field Filter Toolbar):**
  ...
  - **ตัวกรองช่วงวันที่ & ปุ่มลัด (Date Range Filter & Quick Presets - DD/MM/YYYY):** คัดกรองงานตามกำหนดวันนัดหมาย (`plan_date`) ที่แสดงในตาราง ผ่าน Flatpickr ปฏิทินแบบกำหนดช่วง `DD/MM/YYYY` พร้อมปุ่มลัดเลือกช่วง **"วันนี้"** และ **"เดือนนี้"** และปุ่มล้างวันที่ `[X]` ช่วยให้ผู้ตรวจหาคิวงานตามวันนัดหมายได้อย่างรวดเร็วในคลิกเดียว และจัดเรียงลำดับงานตามวันที่นัดหมาย (Chronological) เมื่อเปิดใช้งานตัวกรองวันที่
...
graph LR
    ...
    Review -->|อนุมัติผ่าน 100%| STK["ส่งผลตรวจไป STK (ปิดงานสมบูรณ์)"]
```
...
### 1.1 การส่งต่องานอัตโนมัติจาก Active Input Workspace สู่คิวตรวจ QC (Active Workspace Fast Routing to QC)
- เมื่อช่างหรือเจ้าหน้าที่เปิดดูใบงานด่วนในพื้นที่ **Active Input Workspace** (`JobActiveWorkspace` บนแท็บ "งาน/Task") และกดปุ่ม **"อัปเดตและบันทึกข้อมูล"** บนแถบ Sticky Bottom Action Bar:
  - ระบบจะบันทึกรูปถ่ายมาตรฐาน 5 สล็อต (`PhotoSlots5`) และบันทึกความคิดเห็น/Quick Tags เข้าสู่ Activity Timeline
  - ระบบจะปรับปรุงสถานะงานเป็น **`QC_PENDING` (รอตรวจ Online)** โดยอัตโนมัติ พร้อมตั้งค่าความคืบหน้ารวมเป็นอย่างน้อย 85% และบันทึกประทับเวลา `step_timestamps.qc_pending_at`
  - นำทางและย้ายขั้นตอนไปยัง **คิวตรวจรับรองคุณภาพ QC (Tab `qc` / Step 6)** ทันที
...
## ⭐ มาตรฐานการประเมินคุณภาพ QC รายงานหลัก และการส่งมอบระบบ STK (Per-Task QC & STK Submission)

ระบบ PMT Flow กำหนดมาตรฐานแบบฟอร์มการตรวจประเมินคุณภาพ QC (Step 5) สำหรับงานโครงการ Renovate โดยใช้หลักการ **Per-Task QC & STK Submission (แยกตรวจและปิดงานรายงานหลัก)**:

```mermaid
graph TD
    Project["งานโครงการ Renovate (มีหลายงานหลัก)"]
    
    subgraph Task1["งานหลักที่ 1 (เช่น งานทาสีอาคาร)"]
        P1["ภาพถ่ายช่าง 5 รูป (Before, During 1-2, Test, After)"]
        Q1["1 ข้อมาตรฐาน On-site + เพิ่มข้อคำถามได้เอง"]
        Sub1["รวมงานย่อย (Subtasks) ภายใต้งานทาสี"]
        Btn1["ปุ่ม: บันทึกปิดงานทาสี & ส่งประเมิน STK"]
        P1 --> Q1 --> Sub1 --> Btn1
    end

    subgraph Task2["งานหลักที่ 2 (เช่น งานปูกระเบื้อง)"]
        P2["ภาพถ่ายช่าง 5 รูป (Before, During 1-2, Test, After)"]
        Q2["1 ข้อมาตรฐาน On-site + เพิ่มข้อคำถามได้เอง"]
        Sub2["รวมงานย่อย (Subtasks) ภายใต้งานปูกระเบื้อง"]
        Btn2["ปุ่ม: บันทึกปิดงานปูกระเบื้อง & ส่งประเมิน STK"]
        P2 --> Q2 --> Sub2 --> Btn2
    end
    ...
    Btn1 -->|ยิง REST API ราย Task| STK1["ระบบ STK: STK-TASK-... (งานทาสี)"]
    Btn2 -->|ยิง REST API ราย Task| STK2["ระบบ STK: STK-TASK-... (งานปูกระเบื้อง)"]
    
    STK1 --> Complete{"ส่งครบทุกงานหลัก?"}
    STK2 --> Complete
    Complete -->|ใช่ ครบ 100%| Closed["สถานะโครงการ: QC_PASSED (100% Closed)<br/>*จบขั้นตอนใน PMT Flow / ระบบ STK ดูแล CSAT ต่อเอง*"]
```

### รายละเอียดองค์ประกอบการประเมินในแต่ละงานหลัก:
1. **การตรวจสอบภาพถ่ายช่าง 5 รูป (5 Technician Daily Photos):**
   - ดึงภาพถ่าย 5 รูปที่ช่างหน้างานบันทึกไว้ในบันทึกงานประจำวัน (Daily Work Log) ของ Task นั้นๆ (Before, During 1, During 2, Safety Testing, After) มาแสดงในหัวข้อ **"📸 ภาพถ่ายหน้างาน 5 รูปจากช่าง (Daily Work Log)"**
   - ผู้ตรวจสามารถคลิกรูปเพื่อเปิด **Lightbox Preview** ขยายดูภาพความละเอียดสูงพร้อมเวลาบันทึกได้ทันที
2. **ข้อคำถามมาตรฐาน 1 ข้อ + เพิ่มข้อคำถามได้เอง (1 Standard Question + Dynamic Custom Questions):**
   - ระบบกำหนดข้อคำถามมาตรฐาน On-site เริ่มต้น 1 ข้อ:
     > *"ช่างปฏิบัติงานติดตั้งได้ตามแบบ BOQ และมาตรฐาน On-site ถูกต้องเรียบร้อย"*
   - ผู้ตรวจสามารถพิมพ์ข้อความในช่องอินพุตและคลิกปุ่ม **`➕ เพิ่มข้อคำถามประเมิน`** เพื่อเพิ่มเกณฑ์ตรวจเฉพาะด้านสำหรับงานหลักนั้นๆ ได้ไม่จำกัด
   - ในแต่ละข้อคำถาม สามารถกดลบข้อคำถามเพิ่มเติมได้ (ยกเว้นข้อคำถามมาตรฐานหลักข้อที่ 1)
3. **การประเมินผล Yes / No:**
   - **`✓ Yes — ผ่านเกณฑ์ (5 คะแนน)`**: งานถูกต้อง ได้มาตรฐานในรอบแรก
   - **`✓ Yes — ผ่านเกณฑ์รอบแก้ (1 คะแนน)`**: งานที่เคยสั่งแก้ไข เมื่อแก้ไขเสร็จแล้วตรวจผ่านในรอบถัดไป จะได้รับ 1 คะแนนตามกฎ First-time Pass
   - **`✕ No — ข้อบกพร่อง (1 คะแนน)`**: งานไม่ผ่านเกณฑ์ หรือพบจุดบกพร่องที่ต้องสั่งแก้ไข
   - มีปุ่มลัด **`[ ✓ ตอบ Yes ทุกข้อ ]`** และ **`[ ↺ รีเซ็ตคำตอบ ]`**
4. **งานย่อย (Subtasks) รวมอยู่ในงานหลัก:**
   - รายการงานย่อย (Subtasks) และรายการค่าแรงจาก BOQ จะถูกรวบรวมเข้ามาแสดงผลในกล่อง **"📋 รายการงานย่อยและขอบเขตงาน (Subtasks & Scope)"** เพื่อให้ผู้ตรวจเห็นภาพรวมของงานหลักนั้นครบถ้วน
5. **การกดปิดงานราย Task ส่ง STK ทันที (Task-Level Closure):**
   - แต่ละงานหลักมีกล่องสรุปผลคะแนนและปุ่มปิดงาน:
     > **`✓ บันทึกปิดงาน [ชื่องานหลัก] & ส่งประเมิน STK`**
   - เมื่อกดบันทึกและอนุมัติ ระบบจะส่งข้อมูลผลตรวจของงานหลักนั้นไปยังระบบ STK ทันที และออกรหัสอ้างอิง `STK-TASK-YYYY-XXXXXX` โดยไม่ต้องรอให้งานหลักอื่นเสร็จสิ้นก่อน
6. **การปิดโครงการสมบูรณ์ 100% (No CSAT in PMT Flow):**
   - เมื่อทุกงานหลักได้รับการตรวจและส่ง STK ครบถ้วนแล้ว สถานะของทั้งโครงการจะเปลี่ยนเป็น **`QC_PASSED` (100% Closed)**
   - ระบบ PMT Flow จะสิ้นสุดกระบวนการที่การส่ง STK โดยสมบูรณ์ โดยขั้นตอนการประเมินความพึงพอใจลูกค้า (CSAT) และการวางบิลจะดำเนินการต่อที่ระบบภายนอก STK
   - แนบรูปถ่ายหลักฐานเฉพาะข้อและระบุหมายเหตุรายข้อได้ตามปกติ
7. **🔒 กฎเหล็กการตรวจสอบความเสร็จสิ้นจากแผนงาน Gantt ก่อนตรวจ QC (Gantt Task Completion Gating):**
   - **หลักการสำคัญ:** งานหลักในโครงการ Renovate ที่ช่างยังดำเนินการติดตั้งไม่เสร็จสิ้นตามแผนงาน Gantt Timeline (สถานะงานในตาราง Gantt ยังไม่ใช่ `DONE` หรือความคืบหน้า Daily Work Log ยังไม่ถึง 100%) **ฝ่าย QC จะยังไม่สามารถทำการประเมินข้อคำถาม หรือกดส่งปิดงานไปยังระบบ STK ได้โดยเด็ดขาด**
   - **การแสดงผลเมื่อล็อก (Locked State):**
     - **แท็บงานหลัก:** แสดงป้ายเตือน `⏳ ช่างกำลังทำใน Gantt`
     - **หัวการ์ดงานหลัก:** แสดงป้ายเตือน `🔒 ยังไม่เสร็จสิ้นใน Gantt (สถานะ: กำลังทำ)`
     - **แบนเนอร์แจ้งเตือน:** แสดงกล่องแจ้งเตือนสีเหลือง-ส้มระบุชัดเจนว่า *งานหลักนี้ยังอยู่ระหว่างดำเนินการในแผนงาน Gantt จึงล็อกการประเมินผล QC ชั่วคราว*
     - **ปุ่มประเมิน & รูปภาพ:** ปุ่ม Yes / No ถูก Disabled, ปุ่มอัปโหลดรูปภาพถูกซ่อน, ช่องหมายเหตุถูกล็อก Read-Only, ปุ่ม "เลือก Yes ทั้งหมด" และ "ล้างคำตอบ" ถูกแทนที่ด้วยป้าย `🔒 รอตาราง Gantt บันทึกเสร็จสิ้นก่อนประเมิน`
     - **ปุ่มปิดงานส่ง STK:** ปุ่มบันทึกปิดงานส่ง STK ของ Task และปุ่มส่ง STK ที่ Footer โมดอลจะถูกล็อก (Disabled) ด้วยข้อความ `🔒 รอตาราง Gantt บันทึกเสร็จสิ้นก่อนส่งประเมิน STK`
   - **การปลดล็อก (Unlocked State):** เมื่องานหลักดังกล่าวในตาราง Gantt ถูกปรับสถานะเป็น `DONE` (เสร็จสิ้น) หรือทีมช่างบันทึกงานประจำวัน (Daily Work Log) จนได้ความคืบหน้า 100% ระบบจะปลดล็อกให้ QC ประเมินผล Yes/No แนบรูปภาพ และกดปิดงานส่งข้อมูลเข้าสู่ระบบ STK ได้ทันที
...
> [!TIP]
> **Q: เมื่อช่างหรือเจ้าหน้าที่กดปุ่ม "อัปเดตและบันทึกข้อมูล" ใน Active Input Workspace บนงาน Quick Service งานจะไปที่ใด?**  
> **A:** ระบบจะปรับปรุงสถานะงานเป็น `QC_PENDING` (รอตรวจ QC Online, ความคืบหน้าอย่างน้อย 85%) และนำทางส่งต่องานมายังคิวตรวจคุณภาพ QC (Tab `qc` / Step 6) ทันที พร้อมแสดง Toast แจ้งเตือน: `⚡ งานด่วน (Quick Service) อัปเดตข้อมูลและส่งต่อไปยังขั้นตอนตรวจ QC เรียบร้อยแล้ว`
