...
## ⭐ การประเมินและให้คะแนนแยกตามงานย่อย BOQ (Sub-Task Evaluation & Scoring)

ในงานโครงการ Renovate หนึ่งโครงการจะประกอบด้วยงานย่อยหลายรายการตามที่ถอดจาก BOQ (เช่น งานรื้อถอน, งานก่อฉาบ, งานปูกระเบื้อง, งานสุขภัณฑ์) เจ้าหน้าที่ QC จะต้องทำการตรวจสอบและประเมินผลทีละงานย่อยอย่างละเอียด

```mermaid
graph TD
    BOQ["รายการงานย่อยที่นำเข้าจาก BOQ"]
    
    subgraph ST1["งานย่อยที่ 1: รื้อถอนสุขภัณฑ์เดิม"]
        R1["ให้คะแนน 1-5 ดาว ⭐⭐⭐⭐⭐"]
        S1["สถานะ: PASS / DEFECT"]
        P1["แนบรูปถ่ายหลักฐานเฉพาะงาน"]
    end

    subgraph ST2["งานย่อยที่ 2: ปูกระเบื้องพื้นและผนัง"]
        R2["ให้คะแนน 1-5 ดาว ⭐⭐⭐⭐⭐"]
        S2["สถานะ: PASS / DEFECT"]
        P2["แนบรูปถ่ายหลักฐานเฉพาะงาน"]
    end

    subgraph ST3["งานย่อยที่ 3: ติดตั้งสุขภัณฑ์และอุปกรณ์"]
        R3["ให้คะแนน 1-5 ดาว ⭐⭐⭐⭐⭐"]
        S3["สถานะ: PASS / DEFECT"]
        P3["แนบรูปถ่ายหลักฐานเฉพาะงาน"]
    end
...
### รายละเอียดองค์ประกอบการประเมินในแต่ละงานย่อย:
1. **การให้คะแนนระดับ 1-5 ดาว (Star Rating System):**
   - คลิกที่ดาวเพื่อบันทึกคะแนนคุณภาพของงานย่อยนั้นๆ
   - ⭐⭐⭐⭐⭐ (5 ดาว) = งานสมบูรณ์แบบ ได้มาตรฐาน ไร้จุดตำหนิ (Zero-Defect)
   - ⭐⭐⭐⭐ (4 ดาว) = งานเรียบร้อยดี มีจุดเก็บงานเพียงเล็กน้อย
   - ⭐⭐⭐ (3 ดาว) = งานผ่านเกณฑ์ขั้นต่ำ ต้องกำชับการเก็บความสะอาด
   - ⭐⭐ หรือ ⭐ (1-2 ดาว) = งานไม่ได้มาตรฐาน ต้องสั่งปรับปรุงแก้ไข
2. **การระบุสถานะงานย่อย (Pass / Defect Status):**
   - ปุ่มสีเขียว **`ผ่าน (PASS)`**: งานย่อยนี้ได้มาตรฐานตามแบบแปลน
   - ปุ่มสีแดง **`มีข้อบกพร่อง (DEFECT)`**: งานย่อยนี้ไม่ผ่านเกณฑ์ ต้องมีการแก้ไข
3. **การแนบรูปถ่ายหลักฐานเฉพาะแต่ละงานย่อย (Dedicated Sub-task Photo Evidence):**
   - แต่ละงานย่อยมีพื้นที่อัปโหลดรูปภาพเฉพาะของตนเอง (รูปก่อน-ระหว่าง-หลังทำ)
   - มีปุ่มจำลองรูปถ่ายตัวอย่าง หรืออัปโหลดไฟล์ภาพจริง
   - เมื่อคลิกที่ภาพ จะเปิด **Lightbox Preview** ขนาดใหญ่เพื่อตรวจสอบรอยต่อ ยาแนว หรือความเรียบร้อยของพื้นผิว
4. **บันทึกหมายเหตุของผู้ตรวจ (Inspector Remarks):**
   - บันทึกคำแนะนำ ข้อสังเกต หรือข้อสั่งการเฉพาะสำหรับงานย่อยนั้น เช่น *"ระดับความลาดเอียงของท่อน้ำทิ้งได้ระดับ 1:100 ทดสอบปล่อยน้ำแล้วระบายได้เร็ว"*
...
> สำหรับโครงการที่มีรายการงานย่อย เจ้าหน้าที่ QC **ต้องทำการประเมินและให้ผลรับรองงานย่อยครบทุกรายการ (100% Completion)** ระบบจึงจะยอมให้กดส่งต่อโครงการได้

### ลักษณะการทำงานของระบบ Gating:
1. **การ์ดแสดงความคืบหน้า (Evaluation Progress Bar):**
   - ด้านบนของรายการงานย่อยจะมีการ์ดระบุสัดส่วนการตรวจ เช่น `ประเมินแล้ว 2 จาก 4 งาน (50%)` พร้อมแถบสถานะสีม่วง
2. **ปุ่มอนุมัติถูกล็อก (Disabled State):**
   - ตราบใดที่ยังตรวจไม่ครบ 100% ปุ่ม **"✓ อนุมัติผ่านเกณฑ์ QC & ส่งต่อ CSAT (Step 7)"** จะอยู่ในสถานะปิดการใช้งาน (ปุ่มสีเทา-ขอบประ) และแสดงข้อความเตือน:  
     `⚠️ กรุณาประเมินและให้คะแนนงานย่อยให้ครบทุกงาน (100%) ก่อนส่งต่อ CSAT`
3. **การปลดล็อกปุ่มอนุมัติ (Active State):**
   - เมื่อประเมินงานย่อยครบทุกรายการ แถบสถานะจะเปลี่ยนเป็นสีเขียว `ประเมินครบถ้วน 4/4 งาน (100%)` และปุ่มอนุมัติจะเปลี่ยนเป็นสีเขียวสว่างสดใสพร้อมให้กดส่งต่อทันที
