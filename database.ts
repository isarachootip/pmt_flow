...
// =============================================================================
// AUDIT LOGS REPOSITORY (Append-only & Asia/Bangkok Timezone)
// =============================================================================
...
export async function dbSaveAuditLog(entry: AuditLogEntry): Promise<void> {
  try {
...
export async function dbLoadAuditLogs(filters?: {
  booking_no?: string;
  entity_type?: string;
  entity_id?: string;
  action?: string;
  ...
  try {
    ...
    const whereSql = whereClauses.length > 0 ? `WHERE ${whereClauses.join(' AND ')}` : '';
    const countRes = await pool.query(`SELECT COUNT(*)::int AS total FROM core_audit_logs ${whereSql}`, params);
    const total = Number(countRes.rows[0]?.total) || 0;
    ...
    `;
    const res = await pool.query(querySql, [...params, limit, offset]);
    return {
      logs: res.rows.map(r => ({
        ...r,
...
export async function dbGetJobByBookingNo(bookingNo: string): Promise<any | null> {
  try {
    if (!bookingNo || !bookingNo.trim()) return null;
    const res = await pool.query(
      'SELECT * FROM core_jobs WHERE booking_no = $1 OR external_ref_id = $1 OR job_no = $1 LIMIT 1',
      [bookingNo.trim()]
    );
    if (res.rows.length === 0) return null;
    return mapDbJobRow(res.rows[0]);
...
// =============================================================================
// STK OUTBOUND SYNC LOGS & QUEUE
// =============================================================================

export interface StkSyncLogEntry {
  id?: number;
  ...
  assigned_qc?: string | null;
  final_score?: number;
  rework_count?: number;
  status: 'PENDING' | 'SENT' | 'SYNC_FAILED' | 'SYNCED';
  payload?: any;
  request_payload?: any;
  response_body?: any;
...
export async function dbSaveStkSyncLog(syncLog: StkSyncLogEntry): Promise<StkSyncLogEntry> {
  try {
...
export async function dbUpdateStkSyncLog(idOrKey: string | number, updates: Partial<StkSyncLogEntry>): Promise<void> {
  try {
    ...
    await pool.query(`UPDATE stk_sync_logs SET ${setClauses.join(', ')} WHERE ${whereClause}`, values);
  } catch (err: any) {
    console.error('[DB STK LOG] Error updating STK sync log:', err.message);
  }
}

export async function dbGetStkSyncLog(idOrKey: string | number): Promise<any | null> {
  try {
    ...
    const res = await pool.query(`SELECT * FROM stk_sync_logs WHERE ${whereClause} LIMIT 1`, [idOrKey]);
    return res.rows[0] || null;
  } catch (err: any) {
...
export async function dbGetStkSyncLogByIdempotencyKey(key: string): Promise<any | null> {
  try {
    const res = await pool.query('SELECT * FROM stk_sync_logs WHERE idempotency_key = $1 LIMIT 1', [key]);
    return res.rows[0] || null;
  } catch (err: any) {
...
export async function dbLoadStkSyncLogs(filters?: { status?: string; booking_no?: string; limit?: number }): Promise<any[]> {
  try {
    const whereClauses: string[] = [];
    const params: any[] = [];
    let idx = 1;
    ...
    const res = await pool.query(
      `SELECT * FROM stk_sync_logs ${whereSql} ORDER BY created_at DESC, id DESC LIMIT $${idx}`,
      params
    );
    return res.rows;
  } catch (err: any) {
