...
export async function dbUpdateJob(jobNoOrId: string | number, updates: any): Promise<void> {
  ...
  try {
    ...
    if (updates.status !== undefined) {
      await pool.query(`UPDATE core_jobs SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE ${whereCol} = $2`, [updates.status, jobNoOrId]);
    }
    if (updates.overall_progress !== undefined) {
      await pool.query(`UPDATE core_jobs SET overall_progress = $1, updated_at = CURRENT_TIMESTAMP WHERE ${whereCol} = $2`, [updates.overall_progress, jobNoOrId]);
    }
    if (updates.photos !== undefined) {
      await pool.query(`UPDATE core_jobs SET photos = $1, updated_at = CURRENT_TIMESTAMP WHERE ${whereCol} = $2`, [JSON.stringify(updates.photos), jobNoOrId]);
    }
    if (updates.tasks !== undefined) {
      await pool.query(`UPDATE core_jobs SET tasks = $1, updated_at = CURRENT_TIMESTAMP WHERE ${whereCol} = $2`, [JSON.stringify(updates.tasks), jobNoOrId]);
    }
    if (updates.step_timestamps !== undefined) {
      await pool.query(`UPDATE core_jobs SET step_timestamps = $1, updated_at = CURRENT_TIMESTAMP WHERE ${whereCol} = $2`, [JSON.stringify(updates.step_timestamps), jobNoOrId]);
    }
  } catch (err: any) {
